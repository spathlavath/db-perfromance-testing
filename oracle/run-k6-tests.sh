...
show_menu() {
    ...
    echo ""
    echo -e "${GREEN}1)${NC} LOW Intensity    - 50 VUs       | 1-2s think time    | 10 min"
    echo -e "${GREEN}2)${NC} MEDIUM Intensity - 200 VUs      | 0.5-1s think time   | 10 min"
    echo -e "${GREEN}3)${NC} HIGH Intensity   - 500 VUs      | 0.2-0.5s think time | 10 min"
    echo -e "${YELLOW}4)${NC} STRESS Test      - 1000 VUs     | 0.1-0.3s think time | 10 min"
    echo -e "${RED}5)${NC} MAXIMUM/CRASH    - 2000 VUs     | 0.05-0.2s think time | 10 min"
    echo ""
