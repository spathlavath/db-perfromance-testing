...
show_menu() {
    ...
    echo ""
    echo -e "${GREEN}1)${NC} LOW Intensity    - 0→100 VUs ramping  | 1-2s think time    | 40 min"
    echo -e "${GREEN}2)${NC} MEDIUM Intensity - 0→200 VUs ramping  | 0.5-1s think time  | 40 min"
    echo -e "${GREEN}3)${NC} HIGH Intensity   - 0→500 VUs ramping  | 0.2-0.5s think time | 40 min"
    echo -e "${YELLOW}4)${NC} STRESS Test      - 0→1000 VUs ramping | 0.1-0.3s think time | 40 min"
    echo -e "${RED}5)${NC} MAXIMUM/CRASH    - 0→2000 VUs ramping | 0.05-0.2s think time | 40 min"
    echo ""
